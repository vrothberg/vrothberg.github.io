...
In this short blog post, I want to describe how I believe having tamed the beast of daily mails coming into my inbox from GitHub and how I use mail filters to instantly see which issues, pull requests, comments and discussions I need to address first.

### GitHub Notifications

GitHub provides a rather simple way of dealing with notifications as, at the time of writing, it knows only two categories: **participating** and **watching**.  According to the [documentation](https://help.github.com/articles/about-notifications/) I will receive *participating* notifications when I am directly involved in activities or conversations within a repository or team that I am a member of.  The documentation further states that I will receive *watching* notifications for updates in repositories or team discussions that I am watching [and that do not qualify as participating notifications].  Please refer to the [documentation](https://help.github.com/articles/about-notifications/) for more details.


### Notifications via Email

The GitHub notifications can be watched in a nice web interface on GitHub and, at wish, they can also be send via email to a specified email address.  Shortly after using GitHub more professionally, I switched to using email only as the web interface just did not scale for me.  Using mails for notifications has many benefits for me but the three most important ones are that **1)** I have mail threads which give me context and allow to easily catch up, **2)** I can watch the notifications on any device in any format, and **3)** I can reply via email which scales much better (although I rarely do that to be honest).

After having changed to mail notifications only, I was exposed to a new threat: mails, thousands of them, more than I ever had on the Linux Kernel mailing list.  I really hate distractions and obviously I could not be interested in or scale to hundreds of emails per day about all kinds of topics in projects that I am involved in. The solution was to use **email filters** and to come up with something similar to GitHub's *participating* classification (i.e., important mails) that was lost when switching to email. To make it short, I use the following setup:
